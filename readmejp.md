...
-   **2 段階の PDF 作成:**
    ...
    2.  **PDF への変換:** 次に、`convert()` メソッドがこのバッファを処理し、`printpdf` を使用して最終的な PDF を生成します。なお `printpdf` バージョン 0.8の機能を使用していますので、それが公開されるまでは、githubの最新をダウンロードして使用願います。 また 文字列の幅の計算に `genpdf` を一部改変して（ソースに含まれています）使用しています。
